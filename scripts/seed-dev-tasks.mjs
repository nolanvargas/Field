...
import {
  bulkCountForCurated,
  loadCuratedSeedTasks,
  syncCuratedFilesToStorage,
} from "./lib/curatedTasks.mjs";
import {
  insertSeedTask,
  loadAttachmentTypeIdsBySlug,
  loadTaskCustomFieldDefsSnapshot,
  loadTaskTypeIds,
} from "./lib/insertSeedTask.mjs";
import {
...
function parseTargetCount() {
  ...
  if (!Number.isInteger(n) || n < 0) {
    throw new Error(
      `--count must be a non-negative integer (got ${raw ?? "(missing)"})`,
    );
  }
  return n;
}
...
async function main() {
  ...
  let handTasks = await loadCuratedSeedTasks(anchorMs);
  if (handTasks.length === 0) {
    ...
    handTasks = BASE_TASKS;
  }
  const bulkCount = bulkCountForCurated(targetCount, handTasks.length);
  ...
  try {
    ...
    const TASKS = enrichAllSeedTasks(handTasks, {
      pt,
      ptGridDay,
      addDays,
    ...
    if (dryRun) {
      ...
      console.log(
        `Dry run: would wipe tasks and insert ${TASKS.length} seeds (${handTasks.length} curated/hand + ${bulkCount} generated)`,
      );
      console.log(
        "Statuses:",
        Object.fromEntries(
    ...
    await syncCuratedFilesToStorage();
    console.log(
      `Wrote ${allSeedStorageKeys().length} seed fixture files to ./storage (plus curated pool)`,
    );
    ...
    console.log(
      `Inserted ${TASKS.length} tasks (${handTasks.length} curated/hand + ${bulkCount} generated; 1 soft-deleted).`,
    );
...
const isSeedMain =
  process.argv[1] &&
  path.resolve(process.argv[1]) === path.resolve(__seedDir, "seed-dev-tasks.mjs");
